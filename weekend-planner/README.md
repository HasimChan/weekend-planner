...
# 把 pwa/ 目录推到 gh-pages 分支（或 main 分支的 /docs 子目录）
cd pwa
git init
git add .
...
GitHub Pages 设置：Settings → Pages → Source = `Deploy from a branch` → Branch = `main` / `(root)`

5 分钟后访问 `https://<你的用户名>.github.io/weekend-planner/`，免费 + https + 全球可访问。
